...
def _get_deps(
    ...
    for dep in base_deps:
        Parserd = regex.match(dep).groupdict()  # type: ignore[union-attr]
        dep_name = Parserd["dist"].lower()
        ...
        for pkg_name in lock_pkgs:
            if pkg_name in deps:
                for pkg_dependency in lock_pkgs[pkg_name].get(
                    "dependencies", []):
                    Parserd = regex.match(pkg_dependency).groupdict()  # type: ignore[union-attr]
                    dep_name = Parserd["dist"].lower()
